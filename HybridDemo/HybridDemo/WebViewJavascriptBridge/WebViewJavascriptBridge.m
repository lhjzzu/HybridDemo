...
{
    //javascriptCommand 对应的js命令
    //返回执行命令后的结果
//    NSLog(@"javascriptCommand111 == %@",javascriptCommand);
    //stringByEvaluatingJavaScriptFromString当UIWebView\WKWebView加载完成之后（webViewDidFinishLoad）去调用，才能调用stringByEvaluatingJavaScriptFromString
...
- (BOOL)webView:(UIWebView *)webView shouldStartLoadWithRequest:(NSURLRequest *)request navigationType:(UIWebViewNavigationType)navigationType {
    ...
    if ([_base isCorrectProcotocolScheme:url]) {
        if ([_base isCorrectHost:url]) {
            //注入js，获取注入的命令
            NSString *str = [_base webViewJavascriptFetchQueyCommand];
            //返回js执行后的结果
            NSString *messageQueueString = [self _evaluateJavascript:str];
            //对返回后的结果进行处理
            [_base flushMessageQueue:messageQueueString];
        } else {
            [_base logUnkownMessage:url];
